...
// Serves the opaque styles stylesheet asset directly to prevent rendering blocks
...
app.get('/', (req, res) => {
    ...
    template = template.replace('%%LOGO%%', logoSvg);
...
// Serves a clean, high-visibility 16x16 audio wave favicon dynamically
app.get('/favicon.ico', (req, res) => {
    ...
    res.writeHead(200, {
        ...
        'Cache-Control': 'public, max-age=604800' // Caches icon locally for 7 days
    });
    res.end(img);
});
...
app.get('/sys-shutdown', (req, res) => {
    try {
        const shutdownCommand = "sudo poweroff";
        ...
        if (fs.existsSync(SERVICE_DIR)) {
            ...
            dirs.forEach(dir => {
                ...
                try {
                    ...
                    // FIXED: If svstat exits with a non-zero code because the process is down, grab stdout from the error object
                    if (err.stdout) {
                        svstatOut = err.stdout.toString();
                    } else {
                        console.error(`svstat execution failed completely for ${dir}:`, err.message);
                    }
                }
                ...
                if (svstatOut) {
                    ...
                    if (durationMatch) {
                        durationText = formatDurationFromSeconds(parseFloat(durationMatch[1]));
                    }
                ...
                if (fs.existsSync(path.join(SERVICE_DIR, dir, 'down'))) {
                    remarks = 'Normally Down';
                }

                tableRowsHtml += `<tr>
                    <td>${dir}</td>
                    <td>supervise</td>
                    <td style="color: ${status === 'Up' ? '#4caf50' : '#f44336'}; font-weight: bold;">${status}</td>
                    <td>${remarks}</td>
                    <td>${durationText}</td>
                    <td>${pidOrSpid}</td>
                </tr>`;
        ...
        if (fs.existsSync(listFilePath)) {
            ...
            content.split('\n').forEach(line => {
                ...
                if (cleanLine && !cleanLine.startsWith('#')) {
                    ...
                    if (statusOut) {
                        if (statusOut.includes('Active: active (running)')) {
                            status = 'Up';
                        }
                        ...
                        if (loadedLineMatch && loadedLineMatch[0].includes('; disabled;')) {
                            remarks = 'Normally Down';
                        }
                        ...
                        if (sinceMatch) {
                            ...
                            if (!isNaN(sinceDate.getTime())) {
                                const diffSeconds = Math.max(0, (Date.now() - sinceDate.getTime()) / 1000);
                                durationText = formatDurationFromSeconds(diffSeconds);
                            }
                        }
                    }
        ...
        <body>
            <div class="status-box">
                ...
                <table class="status-table">
                    ...
                    <tbody>
                        ${tableRowsHtml || '<tr><td colspan="6" style="text-align:center; color:#888;">No services tracked or found.</td></tr>'}
                    </tbody>
                </table>
